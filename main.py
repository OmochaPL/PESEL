...
'''
t = input("Ile numerów chcesz sprawdzić? : ")

for i in range(int(t)):
    pesel = input('Podaj numer pesel: ')
    if pesel[10]:
        sumilo = ((int(pesel[0]))*1) + ((int(pesel[1])*1) *3) + ((int(pesel[2]))*7) + ((int(pesel[3]))*9) + ((int(pesel[4]))*1) + ((int(pesel[5]))*3) + ((int(pesel[6]))*7) + ((int(pesel[7]))*9) + ((int(pesel[8]))*1) + ((int(pesel[9]))*3) + ((int(pesel[10]))*1)
        if int(sumilo) % 10 == 0:
            print("D")
        else:
            print("N")
